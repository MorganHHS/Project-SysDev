...
void setup() {
  Serial.begin(115200); 
  Wire.begin();
  // WiFI verbinding, wordt in de loop opnieuw aangeroept voor verloren verbindingen
  WiFi.mode(WIFI_STA);
...
WiFiClient verbindenPi() {    //client verbinding met de server, wordt opnieuw aangeroepen als server verbinding wegvalt
...
void loop() {
  ...
  while (1) {
    ...
    if (!client.connected()) { // als de client-server vebinding wegvalt
